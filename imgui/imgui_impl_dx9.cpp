...
static bool ImGui_ImplDX9_CheckFormatSupport(LPDIRECT3DDEVICE9 d3d_device, D3DFORMAT format)
{
    LPDIRECT3D9 pd3d = nullptr;
    if (d3d_device->GetDirect3D(&pd3d) != D3D_OK)
        return false;
    D3DDEVICE_CREATION_PARAMETERS param = {};
    D3DDISPLAYMODE mode = {};
    if (d3d_device->GetCreationParameters(&param) != D3D_OK || d3d_device->GetDisplayMode(0, &mode) != D3D_OK)
    {
        pd3d->Release();
        return false;
    }
